...
export default function Overview({ 
  ...
  const handleQuickAppealLetter = () => {
    if (onSetAIInitialPrompt) {
      onSetAIInitialPrompt(
        `Kính thưa Trợ lý Gia tộc, tôi muốn soạn một bức Thư ngỏ thay mặt Ban Trị Sự dòng họ Cao để kêu gọi con cháu chung tay bổ sung tư liệu, ảnh chân dung, ngày giỗ và dữ liệu phả hệ còn khuyết. Hãy viết lời hiệu triệu tha thiết, không tự thêm địa danh hoặc công trình nếu chưa có tài liệu xác minh.`,
        "appeal"
      );
      onSetActiveTab("ai");
    }
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-red-950 via-red-900 to-amber-950 px-6 py-8 text-white shadow-xl border border-amber-950">
        ...
        <div className="relative z-10 max-w-2xl space-y-2">
          <div className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/20 px-3 py-1 text-xs font-medium text-amber-300 border border-amber-500/30">
            ...
            Hệ thống Quản trị Số hóa Gia tộc họ Cao
          </div>
            ...
            {fundStatistics.map((fund, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="w-full min-h-7 bg-stone-200/55 rounded-full overflow-hidden border border-stone-150 flex items-center relative">
                    <motion.div 
                      initial={{ width: 0 }}
                      animate={{ width: `${percentage}%` }}
                      transition={{ duration: 0.8 }}
                      className={`h-full rounded-full cursor-pointer opacity-90 absolute inset-y-0 left-0 ${
                    ...
                    <span className="relative z-10 ml-2 max-w-[calc(100%-1rem)] rounded-full bg-white/90 px-2 py-0.5 text-[10px] text-stone-850 font-extrabold shadow-sm border border-white/80 leading-tight">

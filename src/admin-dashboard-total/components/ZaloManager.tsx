...
interface ZaloManagerProps {
  ...
  initialRules?: ZaloAutoReply[];
  onRulesChange?: (rules: ZaloAutoReply[]) => void;
}
...
export default function ZaloManager({ 
  ...
  useEffect(() => {
    const zaloUsers = usersList.filter((user) => user.loginType === "zalo");
    if (zaloUsers.length === 0) return;

    setSubscribers((prev) => {
      let changed = false;
      const next = [...prev];

      zaloUsers.forEach((user) => {
        const linkedMember = user.linkedMemberId ? members.find((member) => member.id === user.linkedMemberId) : undefined;
        const phoneOrZaloId = user.phone || user.username;
        const oauthSub: ZaloUser = {
          id: `oauth_${user.id}`,
          name: user.fullName || user.username,
          phone: phoneOrZaloId,
          branch: linkedMember?.branch || "Chưa xác định",
          regDate: user.regDate || new Date().toLocaleDateString("vi-VN"),
          status: user.isApproved || user.approvalStatus === "approved" ? "Đang hoạt động" : "Chờ duyệt",
          linkedMemberId: user.linkedMemberId,
          relationship: linkedMember ? "Bản thân" : undefined,
          notes: linkedMember
            ? `Đồng bộ từ tài khoản Zalo đã KYC với ${linkedMember.name}.`
            : "Đồng bộ từ tài khoản đăng nhập Zalo, chờ admin quy chiếu KYC.",
          group: user.roles?.includes("admin") ? "Ban trị sự" : "Khác"
        };

        const index = next.findIndex((sub) => sub.id === oauthSub.id || (!!phoneOrZaloId && sub.phone === phoneOrZaloId));
        if (index === -1) {
          next.unshift(oauthSub);
          changed = true;
          return;
        }
        ...
        const updated = {
          ...existing,
          name: oauthSub.name || existing.name,
          phone: existing.phone || oauthSub.phone,
          branch: linkedMember?.branch || existing.branch || oauthSub.branch,
          status: oauthSub.status,
          linkedMemberId: oauthSub.linkedMemberId || existing.linkedMemberId,
          relationship: oauthSub.relationship || existing.relationship,
          notes: existing.notes || oauthSub.notes,
          group: existing.group || oauthSub.group
        };
  ...
  const [rules, setRulesState] = useState<ZaloAutoReply[]>(() => initialRules.length ? initialRules : defaultAutoRules);

  const setRules = (updater: ZaloAutoReply[] | ((prev: ZaloAutoReply[]) => ZaloAutoReply[])) => {
    setRulesState((prev) => {
  ...
  const [broadcasts, setBroadcasts] = useState<ZaloBroadcast[]>([
    { id: "b1", title: "Kêu gọi bổ sung tư liệu gia phả", sentDate: "26/05/2026", recipientsCount: 125, content: "Kêu gọi con cháu chung tay bổ sung ảnh chân dung, ngày giỗ, chi/ngành và tài liệu gốc để hoàn thiện kho dữ liệu phả hệ họ Cao.", category: "Lời kêu gọi trùng tu" },
  ...
  const [newSubBranch, setNewSubBranch] = useState("Chưa xác định");
  const [showBranchSuggest, setShowBranchSuggest] = useState(false);
  const [newSubGroups, setNewSubGroups] = useState<string[]>(["Thanh niên họ"]);
  ...
  const branchSuggestions = useMemo(() => {
    const values = new Set<string>(["Chưa xác định"]);

    const displayName = (value?: string) => String(value || "").trim();
    members
      .filter((member) => member.parentId && member.gender !== "Nữ")
      .forEach((member) => {
        const name = displayName(member.name);
        if (name) values.add(`Chi ${name}`);
      });

    managedBranches.forEach((branch) => {
      if (branch.name?.trim()) values.add(branch.name.trim());
    });

    members.forEach((member) => {
      const branch = member.branch?.trim();
      if (!branch) return;
      const normalizedBranch = branch.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
      if (/^chi\s+thu\s+/.test(normalizedBranch)) return;
      if (/^chi\s+truong/.test(normalizedBranch)) return;
      values.add(branch);
    });
    return Array.from(values).sort((a, b) => a.localeCompare(b, "vi"));
  }, [managedBranches, members]);

  React.useEffect(() => {
    const reloadManagedBranches = () => {
      try {
        const saved = JSON.parse(localStorage.getItem("caogia_admin_branch_registry_v1") || "[]");
    ...
    window.addEventListener("storage", reloadManagedBranches);
    window.addEventListener("caogia_branch_registry_updated", reloadManagedBranches);
    return () => {
      window.removeEventListener("storage", reloadManagedBranches);
      window.removeEventListener("caogia_branch_registry_updated", reloadManagedBranches);
    };
  }, []);

  const filteredBranchSuggestions = useMemo(() => {
    const term = newSubBranch.trim().toLowerCase();
    if (!term) return branchSuggestions.slice(0, 8);
    const exactMatch = branchSuggestions.some((branch) => branch.toLowerCase() === term);
    const matches = branchSuggestions
      .filter((branch) => branch.toLowerCase().includes(term))
      .slice(0, 8);
    return exactMatch ? matches : [newSubBranch, ...matches.filter((branch) => branch !== newSubBranch)].slice(0, 8);
  }, [branchSuggestions, newSubBranch]);

  const toggleNewSubGroup = (group: string) => {
    setNewSubGroups((prev) => {
      if (prev.includes(group)) {
        const next = prev.filter((item) => item !== group);
        return next.length ? next : ["Khác"];
      }
      return [...prev, group];
    });
  };
  ...
  const handleGenerateCampaignWithAI = async () => {
    ...
    try {
      const response = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          prompt: `Hãy viết một bức thư thông tri dòng họ nghiêm cẩn của dòng họ Cao với tiêu đề "${bcTitle}" và nhóm sự vụ văn bản "${bcCategory}". Lời văn gia nghiêm, chân phương cổ truyền, phù hợp để gửi tin nhắn Zalo hàng loạt tới thành viên dòng họ. Chỉ nhắc địa danh, công trình hoặc nhân vật nếu dữ liệu đầu vào đã nêu rõ.`,
          type: "zalo_campaign",
          botType: "zalo",
          intent: "campaign",
      ...
      setBcContent(`Kính gửi quý bối và con cháu họ Cao,\n\nBan Trị Sự trân trọng thông lạp bản sớ sự vụ: "${bcTitle}".\n\nRất mong toàn tộc bớt chút thời gian theo dõi, đối chiếu và bổ sung thông tin nếu có dữ liệu xác thực.\n\nTần suất chiến dịch: ${bcFrequency}\nLịch phát sóng: ${bcScheduleTime}\n\nKính chúc gia quyến bình khang!`);
    } finally {
              ...
              <form onSubmit={handleSendBroadcast} className="lg:col-span-7 space-y-4 text-xs">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea 
                    rows={5.5}
                    required
                    placeholder="Viết nội dung dâng lời nhắn gửi... Hoặc gõ tiêu đề rồi nhấp nút 'Tạo nội dung bằng AI' để hệ thống tự viết văn phong chuẩn gia giáo dòng họ Cao." 
                    value={bcContent}
            ...
            >
              ...
              <form onSubmit={handleAddNewManualSub} className="p-5 space-y-4 text-xs">
                ...
                <div className="space-y-1 relative">
                  <label className="font-semibold text-stone-700 block">Chi/ngành phả hệ:*</label>
                  <input
                    type="text"
                    required
                    value={newSubBranch}
                    onFocus={() => setShowBranchSuggest(true)}
                    onBlur={() => window.setTimeout(() => setShowBranchSuggest(false), 140)}
                    onChange={(e) => {
                      setNewSubBranch(e.target.value);
                    ...
                    placeholder="Gõ hoặc chọn chi/ngành, ví dụ: Chưa xác định"
                  />
                  {showBranchSuggest && filteredBranchSuggestions.length > 0 && (
                    <div className="absolute z-30 mt-1 w-full max-h-52 overflow-y-auto rounded-lg border border-stone-200 bg-white shadow-xl">
                      {filteredBranchSuggestions.map((branch) => (
                        <button
                          ...
                          onClick={() => {
                            setNewSubBranch(branch);
                            setShowBranchSuggest(false);
                          }}
                          className="block w-full px-3 py-2 text-left text-xs font-semibold text-stone-750 hover:bg-red-50 hover:text-red-900"
                        >
                          {branch}
                        </button>
                      ))}
                  ...
                  <p className="text-[10px] text-stone-400">
                    Gợi ý lấy từ gia phả và danh sách Zalo hiện có; có thể nhập tự do nếu chưa xác định.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="space-y-1">
                    <label className="font-semibold text-stone-700 block">Quan hệ với người KYC:</label>
                    <select
                  ...
                  <div className="space-y-1 sm:col-span-2">
                    <label className="font-semibold text-stone-700 block">Nhóm tham gia:*</label>
                    <div className="flex flex-wrap gap-1.5 rounded border border-stone-200 bg-stone-50 p-2 max-h-24 overflow-y-auto">
                      {zaloGroups.map(g => (
                        <button
                          key={g}
                          type="button"
                          onClick={() => toggleNewSubGroup(g)}
                          className={`rounded-full border px-2.5 py-1 text-[10px] font-bold transition-colors ${
                            newSubGroups.includes(g)
                    ...
                    <p className="text-[10px] text-stone-400">Một người có thể tham gia nhiều nhóm.</p>
                  </div>
                </div>

...
const roleLabels: Record<string, string> = {
  admin: "Chánh Tổng Quản",
  writer: "Sử Biên Ký",
  treasurer: "Thủ Quỹ Gia Tộc",
  secretary: "Thư Ký Họ",
  user: "Thành viên"
};
...
export default function Sidebar({ activeTab, onSelectTab, serverHealth, currentUser, themeConfig }: SidebarProps) {
  ...
  const roleText = roles.map((role) => roleLabels[role]).filter(Boolean).join(" & ");
        ...
        <div className="p-4 space-y-3.5 border-t border-stone-800/40">
          <div className="bg-red-950/20 p-3 rounded-lg border border-red-900/30 flex gap-2.5 items-center">
            ...
            <div className="text-[10.5px] text-stone-400 leading-tight overflow-hidden">
              ...
              <span className="text-[9.5px] text-amber-200 block mt-0.5 truncate font-semibold">
                {roleText || (currentUser.isKYCed ? "Đinh viên đã xác thực" : "Hương đinh tự do")}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-[10px] text-stone-500 px-1 select-none">
            <span className="flex items-center gap-1">
              ...
              Đồng bộ dữ liệu: {serverHealth ? "Bình khang" : "Khuyết"}
            </span>
